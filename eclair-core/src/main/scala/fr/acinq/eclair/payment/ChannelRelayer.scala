...
import fr.acinq.eclair.payment.Relayer.{ChannelUpdates, NodeChannels, OutgoingChannel}
...
class ChannelRelayer(nodeParams: NodeParams, relayer: ActorRef, register: ActorRef, commandBuffer: ActorRef) extends Actor with ActorLogging {
  ...
  override def receive: Receive = {
    ...
    case Status.Failure(addFailed: AddHtlcFailed) => addFailed.origin match {
      case ChannelRelayed(originChannelId, originHtlcId, _, _) => addFailed.originalCommand match {
        case Some(cmd) =>
          ...
          relayer ! ForwardAdd(cmd.upstream.right.get, cmd.previousFailures :+ addFailed)
        case None =>
