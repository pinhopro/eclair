...
object Onion {
  ...
  case class NodeRelayPayload(records: TlvStream[OnionTlv]) extends RelayPayload with TlvFormat {
    ...
    val totalAmount = records.get[TotalAmount].map(_.amount).getOrElse(amountToForward)
    val paymentSecret = records.get[PaymentSecret].map(_.secret)
    val invoiceFeatures = records.get[InvoiceFeatures].map(_.features)
    val invoiceHints = records.get[InvoiceHints].map(_.extraHops)
  }
  ...
  /** Create a trampoline inner payload instructing the trampoline node to relay via a legacy payment. */
  def createNodeRelayToLegacyPayload(amount: MilliSatoshi, totalAmount: MilliSatoshi, expiry: CltvExpiry, targetNodeId: PublicKey, invoice: PaymentRequest): NodeRelayPayload = {
    val tlvs = Seq[OnionTlv](AmountToForward(amount), OutgoingCltv(expiry), OutgoingNodeId(targetNodeId), InvoiceFeatures(invoice.features.bitmask.bytes), InvoiceHints(invoice.routingInfo.toList.map(_.toList)))
    val tlvs2 = invoice.paymentSecret.map(s => tlvs :+ PaymentSecret(s)).getOrElse(tlvs)
    val tlvs3 = if (amount != totalAmount) tlvs2 :+ TotalAmount(totalAmount) else tlvs2
  ...
  def createSinglePartPayload(amount: MilliSatoshi, expiry: CltvExpiry, paymentSecret: Option[ByteVector32] = None): FinalPayload = paymentSecret match {
    case Some(paymentSecret) => FinalTlvPayload(TlvStream(AmountToForward(amount), OutgoingCltv(expiry), PaymentSecret(paymentSecret)))
  ...
  def createTrampolinePayload(amount: MilliSatoshi, totalAmount: MilliSatoshi, expiry: CltvExpiry, paymentSecret: ByteVector32, trampolinePacket: OnionRoutingPacket): FinalPayload = {
    if (amount == totalAmount) {
      FinalTlvPayload(TlvStream(AmountToForward(amount), OutgoingCltv(expiry), PaymentSecret(paymentSecret), TrampolineOnion(trampolinePacket)))
    } else {
      FinalTlvPayload(TlvStream(AmountToForward(amount), TotalAmount(totalAmount), OutgoingCltv(expiry), PaymentSecret(paymentSecret), TrampolineOnion(trampolinePacket)))
    }
  }
...
object OnionCodecs {
  ...
  private val onionTlvCodec = discriminated[OnionTlv].by(varint)
    ...
    .typecase(UInt64(246), invoiceFeatures)
  ...
  val nodeRelayPerHopPayloadCodec: Codec[NodeRelayPayload] = tlvPerHopPayloadCodec.narrow({
    ...
    case tlvs if tlvs.get[OutgoingNodeId].isEmpty => Attempt.failure(MissingRequiredTlv(UInt64(250)))
