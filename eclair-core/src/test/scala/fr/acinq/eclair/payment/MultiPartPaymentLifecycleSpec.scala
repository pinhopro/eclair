...
class MultiPartPaymentLifecycleSpec extends TestKit(ActorSystem("test")) with fixture.FunSuiteLike {
  ...
  test("skip empty channels") { f =>
    ...
    val balances = usableBalances.copy(balances = usableBalances.balances ++ Seq(
      UsableBalance(b, ShortChannelId(42), 0 msat, 1000 * 1000 msat, isPublic = true, channelUpdate_ab_1.copy(shortChannelId = ShortChannelId(42))),
      UsableBalance(e, ShortChannelId(43), 0 msat, 1500 * 1000 msat, isPublic = true, channelUpdate_ab_1)
    ))
    initPayment(f, payment, emptyStats.copy(capacity = Stats(Seq(1000), d => Satoshi(d.toLong))), balances)
    waitUntilAmountSent(f, payment.amount)
    payFsm.stateData.asInstanceOf[PaymentProgress].pending.foreach {
      case (id, payment) => childPayFsm.send(payFsm, PaymentSent(id, paymentHash, paymentPreimage, Seq(PartialPayment(id, payment.finalPayload.amount, 5 msat, randomBytes32, None))))
    }
