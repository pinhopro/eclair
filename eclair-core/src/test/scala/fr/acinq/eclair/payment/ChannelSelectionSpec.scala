...
import fr.acinq.eclair.payment.ChannelRelayer.{RelayFailure, RelaySuccess, relayOrFail, selectPreferredChannel}
...
class ChannelSelectionSpec extends FunSuite {
  ...
  test("convert to CMD_FAIL_HTLC/CMD_ADD_HTLC") {
    ...
    // nominal case
    assert(relayOrFail(relayPayload, Some(channelUpdate)) === RelaySuccess(ShortChannelId(12345), CMD_ADD_HTLC(relayPayload.payload.amountToForward, relayPayload.add.paymentHash, relayPayload.payload.outgoingCltv, relayPayload.nextPacket, upstream = Right(relayPayload.add), commit = true)))
    // no channel_update
